...
void loop()
{
  ...
  if(digitalRead(27)){
    ...
    if(counter == 1){
      ...
      if(setup_executed == 0) {alien.game_setup(); setup_executed = 1; loop_count = 1; total_time_weather = 0;}
      if(setup_executed == 1) alien.game_routine(); 
    ...
    if(loop_count % 100 == 0)
    {
      ...
      if(counter == 1) 
      {
        Serial.print(" Average_alien_task_time: ");
        Serial.print(avg_time_task_wheather);
  
      }else if(counter == 2)
      {
        Serial.print(" Average_loragame_task_time: ");
